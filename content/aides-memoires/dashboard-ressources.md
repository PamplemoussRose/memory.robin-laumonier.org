...
La mise en place du tableau de bord repose sur plusieurs éléments :
...
L'interface va aller chercher les séries créées par la base de données puis les afficher sous forme de graphiques pour les rendre compréhensibles.
...
Une fois le fichier complété, vous pouvez démarrer le service et vérifier qu'il est bien actif :

```sh
sudo systemctl daemon-reload          # Rafraîchi la liste des services
...
Pour télécharger Prometheus et préparer l'environnement, exécutez les commandes suivantes :
...
Nous devons ensuite créer un utilisateur pour Prometheus ainsi que lui donner les permissions sur les dossiers que nous avons juste créé.
...
Une fois l'utilisateur créé et les permissions données, nous pouvons créer le fichier de configuration :
...
```yml
global:
  scrape_interval: 15s # Intervalle de recupération des données. Par défaut 15 secondes.
...
Pour terminer, nous devons créer le fichier service de Prometheus.
...
Le dépôt où se trouve Grafana n'est pas de base dans les dépôt `apt`. Nous devons donc l'ajouter avant de pouvoir télécharger Grafana.
...
Vous pouvez accéder à l'interface Grafana via l'URL <http://localhost:3000>.
...
Il vous sera alors demandé de changer le mot de passe pour les futures connexions.

#### Lien avec Prometheus

Une fois connecté, vous devez lier Grafana avec Prometheus.
