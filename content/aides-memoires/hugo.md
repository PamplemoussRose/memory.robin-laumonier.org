---
title: "Tutoriel Hugo avec Blowfish"
...
Hugo est un framework qui permet de créer des sites web statiques à partir de fichiers en Markdown.  
...
Une fois que Hugo et Git sont installés, nous pouvons initialiser le projet via la commande suivante :

```bash
hugo new site path
```

La variable `path` correspond au chemin du dossier où sera initialisé le projet.
...
Pour stocker le code de notre site, il est recommandé de créer un dépôt Git. De plus, nous allons utiliser Git pour importer le thème Blowfish dans notre projet.
...
Cette commande va importer le dépôt *blowfish.git* dans le dossier `themes/blowfish` ce qui permet d'avoir accès aux fichiers du thème et de personnaliser la disposition des éléments si celle d'origine ne vous plaît pas.

Pour terminer l'installation de Blowfish, il faut :

- Supprimer le fichier `hugo.toml` qui est à la racine du projet
- Copier tous les fichiers *.toml* du dossier `themes/blowfish/config/_default` dans le dossier `config/_default`
...
├─ markup.toml             # Paramètres pour que le thème fonctionne
├─ menus.en.toml           # Personnalisation du header et du footer
├─ params.toml             # Personnalisation des éléments du site
└─ module.toml
```
...
Avant de passer à la personnalisation du site, il reste quelques éléments à régler.
...
Le fichier `languages.en.toml` contient une section concernant les informations de l'auteur du site.  
...
Dans le cas où vous voulez mettre une photo de profil, l'image doit être stockée dans le dossier `assets/img` et le chemin doit être spécifié dans la ligne correspondante.
...
Toutes les palettes sont disponibles à l'adresse suivante : <https://blowfish.page/docs/getting-started/#colour-schemes>
...
Il est également possible de créer sa propre palette.  
...
Pour plus d'informations sur les palettes de couleurs, visitez <https://blowfish.page/docs/advanced-customisation/#colour-schemes>
...
Lors de la création d'une page, il est important de mettre quelques lignes spécifiques au début des fichiers `.md`. Elles permettent d'avoir notamment le nom de la page, le type de disposition ou encore si la page est à prendre en compte ou non.
...
- `title` : Titre de la page. Il sera affiché en gros au debut de la page ainsi que dans le nom de l'onglet.
...
- `draft` : Booléen pour savoir si la page doit être ignorée lors du rendu du site. Une page ignorée ne sera pas disponible sur le site final.

Les `categories` et `tags` sont différenciés par la possibilité de n'afficher que les catégories ou d'afficher les catégories et les tags sur les pages.
...
Chaque élément du menu suit l'un des formats suivant :
...
- `[[menu]]` : Peut être `main` ou `footer`. Correspond au menu auquel sera rattaché l'élément.
...
Un élément parent sans attribut `pageRef` ne va pas être cliquable mais va se dérouler s'il a des éléments enfants.
...
Pour déployer votre site web, vous devez uniquement prendre le dossier `public` qui contient tous les fichiers finaux.
...
Pour plus d'information sur la configuration ou sur la personnalisation du site, consultez la documentation officelle de Blowfish :
